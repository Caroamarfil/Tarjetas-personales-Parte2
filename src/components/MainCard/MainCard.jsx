...
import { cardsData } from '../CardsData/Data';

const MainCard = ({ cardData }) => {
  const { name: cardNameFromUrl } = useParams();
  const card = cardData || cardsData.find((card) => card.name === decodeURIComponent(cardNameFromUrl));
  ...
  return (
    <div className={`card text-center shadow-sm rounded-4 uniform-card ${card.cardClass}`}>
        ...
        <div className="bg-primary p-4 rounded-top-4">
          <div className="position-relative">
            <img
              ...
              alt="Foto de perfil"
              style={card.imgStyle || { width: "130px", height: "130px", objectFit: "cover", marginBottom: "-4rem" }}
            />
          </div>
        </div>
      ) : (
      ...
      <div className="card-body">
        <h5 className={`card-title fw-bold mb-3 ${card.nameClass || ''}`}>{card.name}</h5>
        ...
        {card.tags && card.tags.length > 0 && (
          <div className="d-flex justify-content-center gap-2 mb-3">
            {card.tags.map((tag, index) => (
              <span
                key={index}
                className={tag.className || "badge bg-primary bg-opacity-10 text-primary"}
              >
                {tag.label || tag}
              </span>
            ))}
        ...
        <p className="card-text text-muted fst-italic px-4 mb-4">{card.description}</p>
      ...
      {card.footer && (
        <div className={`card-footer ${card.footerColor || ''}`}>
          {card.footerIcon && (
            <i className={`${card.footerIcon} me-2`}></i>
          )}
          {card.footerText}
        </div>
      )}
